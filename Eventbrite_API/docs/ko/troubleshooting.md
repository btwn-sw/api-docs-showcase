...
실패하는 Eventbrite API 요청의 원인을 찾고 해결하세요. 각 섹션은 특정 오류나
증상, 가장 흔한 원인, 해결 단계를 다룹니다.
...
2. [API Keys page](https://www.eventbrite.com/account-settings/apps)에서
...
일부 작업은 이벤트 상태에 따라 제한됩니다.

### 확인

원래 요청을 다시 보내세요. `200 OK` 응답이 오면 권한 문제가 해결된 것입니다.
...
2. 올바른 Base URL과 API 버전을 사용하고 있는지 확인하세요.
...
`"status": "draft"`이면 이벤트를 게시하거나 테스트용으로
게시된 이벤트를 사용하세요.

2. 코드에서 중첩 필드에 접근하기 전에 Null(널) 체크를 추가하세요.
...
- 신뢰할 수 없는 컨텍스트에서 HTML을 정제 없이 렌더링합니다.

### 해결 방법

1. 알림이나 로그 출력처럼 일반 문자열이 필요한 곳에는
...
3. 신뢰할 수 없는 출처의 HTML을 렌더링한다면 먼저 정제하세요.
...
2. 대량 요청을 보낼 때는 요청 사이에 지연을 추가하세요.
