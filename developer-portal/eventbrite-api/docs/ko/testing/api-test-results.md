...
- [결과 요약](#결과-요약)
- [발견 사항](#발견-사항)
    - [F-001 — `status=both` 문서 불일치](#f-001--statsboth-문서-불일치)
    - [F-002 — `costs` 필드 엔드포인트별 불일치](#f-002--costs-필드-엔드포인트별-불일치)
- [테스트 케이스 및 결과](#테스트-케이스-및-결과)
    - [TC-001 — 단일 주문 조회](#tc-001--단일-주문-조회)
    - [TC-002 — 이벤트별 주문 목록 조회](#tc-002--이벤트별-주문-목록-조회)
    - [TC-003 — status 필터 검증](#tc-003--status-필터-검증)
    - [TC-004 — 내 주문 목록 조회](#tc-004--내-주문-목록-조회)
    - [TC-005 — 이메일 필터 검증](#tc-005--이메일-필터-검증)
